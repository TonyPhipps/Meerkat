Function Get-DomainInfo {
    ...
    process{
        ...
        $GetADReplication = Get-ADReplicationPartnerMetadata -Target $DomainName.DNSRoot
            foreach ($Property in $GetADReplication.PSObject.Properties) {
                $ResultsArray | Add-Member -MemberType NoteProperty -Name $Property.Name -Value $Property.value -ErrorAction SilentlyContinue
            }
        
        $GetDomainComputers = (Get-ADComputer -Filter *).DistinguishedName -join ";" 
            foreach ($result in $GetDomainComputers) 
            {
                $ResultsArray | Add-Member -MemberType NoteProperty -Name "DomainComputers" -Value $GetDomainComputers -ErrorAction SilentlyContinue
            }
        
        $GetDomainUsers = (Get-ADUser -Filter *).DistinguishedName -join ";"
            foreach ($result in $GetDomainUsers) 
            {
                $ResultsArray | Add-Member -MemberType NoteProperty -Name "DomainUsers" -Value $GetDomainUsers -ErrorAction SilentlyContinue
            }
        ...
        return $ResultsArray | Select-Object Host, DateScanned DomainNamingMaster, SchemaMaster, InfrastructureMaster, RIDMaster, 
            ...
            SyncOnStartup, DomainComputers, DomainUsers, Connectivity, Advertising, FrsEvent, DFSREvent, SysVolCheck, KccEvent, 
            KnowsOfRoleHolders, MachineAccount, NCSecDesc, NetLogons, ObjectsReplicated, Replications, RidManager, Services, 
            SystemLog, VerifyReferences, DCDiagDNS-Connectivity, DCDiagDNS-DNS  
    }
